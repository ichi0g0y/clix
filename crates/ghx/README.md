...
GitHub Release is triggered by pushing a `ghx-v<version>` tag. `task release:ghx` fetches `origin/main`, reads the version from `crates/ghx/Cargo.toml` on that branch, creates a temporary worktree at `origin/main`, runs `cargo test -p ghx` there, then creates and pushes the tag. This means you can safely run it from a Conductor worktree or any other local branch.
