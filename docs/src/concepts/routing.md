...
1. **Explicit credentials in the environment** — if the upstream CLI's own
   ...
   environment and clix stays out of the way.
...
|---|---|---|---|---|
| Env passthrough | `GH_TOKEN` (managed by `gh`) | `FLY_API_TOKEN` / `FLY_ACCESS_TOKEN` | `CLOUDFLARE_ACCOUNT_ID` (+ `CLOUDFLARE_API_TOKEN`) | `--api-key`, `STRIPE_API_KEY`, `STRIPE_SECRET_KEY` |
| Override flag | — | `--profile <name>` | — | `-p` / `--project-name` |
| Resource flag | — | `-a` / `--app`, `-o` / `--org` | account id | — |
...
[wranglerx](../tools/wranglerx.md), [stripex](../tools/stripex.md)), because a
few tools add tool-specific rungs (e.g. flyx also matches `-o <org-slug>`).
